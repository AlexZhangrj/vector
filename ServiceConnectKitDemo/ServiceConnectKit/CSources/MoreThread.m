...
#import <ServiceConnectKit/ServiceConnectKit-Swift.h>

#define MAXLEN 1023
#define PORT 6666

void getLocalIP(int8_t ethIndex, char * buffer){
    int inet_sock;
    struct ifreq ifr;
    inet_sock = socket(AF_INET, SOCK_DGRAM, 0);
    
    char ifr_name[32] = {NULL};
    
    snprintf(ifr_name, 31, "en%d\0", ethIndex);
...
//        ev_io* recv_watcher2 = malloc(sizeof(ev_io));
//        ev_io_init(recv_watcher2, write_callback, item->sfd, EV_WRITE);
//        ev_io_start(((WORK_THREAD*)(w->data))->loop, recv_watcher2);
        
//        ev_io_init(recv_watcher2,write_callback,fd,EV_WRITE);
//        ev_io_start(loop,recv_watcher2);

        
        ev_io* recv_watcher = malloc(sizeof(ev_io));
        ev_io_init(recv_watcher, recv_callback, item->sfd,EV_IO);
...
static char localHost[32] = {NULL};

int SeriviceMain()
{
    getLocalIP(0, localHost);
    thread_init();
    int listen;
    listen=socket_init();
...
int socket_init()
{
    int listener = 0;//[SwiftBridge initSocket];
...
//    int newfd = [SwiftBridge acceptSocket];
    
    int newfd = 0;
    
    ev_io* accept_watcher=malloc(sizeof(ev_io));
    ev_io_init(accept_watcher,recv_callback, newfd, EV_READ | EV_WRITE);
...
void SendTestToIO(struct ev_io *w) {
    //    char buffer[1024]={0};
    //    snprintf(buffer,1023,"this is a test!");
    //    write(w->fd, buffer, strlen(buffer));//new
    
    
    char * buffer = malloc(1024 * 1024 + 1);
    ...
    char *begin = buffer;
    size_t remainLen = 1024*1024;
    
    //    snprintf(buffer,1024 * 1024 + 1,"this is a test!");
    ssize_t result = 1;
    result = send(w->fd, begin, remainLen, 0);

//    while (result > 0) {
//        result = send(w->fd, begin, remainLen, 0);
////        result = write(w->fd, begin, remainLen);//new
//        begin += result;
//        remainLen -= result;
...
void writeTestToIO(struct ev_io *w) {
//    char buffer[1024]={0};
//    snprintf(buffer,1023,"this is a test!");
//    write(w->fd, buffer, strlen(buffer));//new
...
void write_callback(struct ev_loop *loop, ev_io *w, int revents)
{
    ...
    //ev_io read_event;
    snprintf(buffer,1023,"this is a test!");
//    write(w->fd,buffer,strlen(buffer),0);//old
    write(w->fd, buffer, strlen(buffer));//new
